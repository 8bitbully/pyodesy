...
class ReferenceEllipsoid(NivoEllipsoid):
    ...
    def __init__(self, ellipsoid: str):
        ellipsoids = self.__ellipsoid(ellipsoid.upper())
        self.Name = ellipsoids["Name"]
        self.SemimajorAxis = ellipsoids["SemimajorAxis"]
        self.SemiminorAxis = ellipsoids["SemiminorAxis"]
        self.InverseFlattening = ellipsoids["InverseFlattening"]

    def __ellipsoid(self, ell):
        ellipsoids = {
            "GRS80":{
...
cosf = lambda rad: cos(rad * fi)
tanf = lambda rad: tan(rad * fi)
atanf= lambda rad: atan(rad* fi)
atan2f= lambda rad1, rad2: atan2(rad1*fi, rad2*fi)

def geog2geoc(ellipsoid, B, L, h, latType = "geographic"):
...
# pollard iteration
def geoc2geog(ellipsoid, x, y, z, type = "degrees"):
    a_ = ellipsoid.SemimajorAxis
    b_ = ellipsoid.SemiminorAxis
    eu2= ellipsoid.SecondEccentricity**2
    ...
    return round(b,5), round(l,5), round(h,3)
